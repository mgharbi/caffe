...
class NormalizedDeconvolutionLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  NormalizedDeconvolutionLayerTest()
  ...
  virtual ~NormalizedDeconvolutionLayerTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(NormalizedDeconvolutionLayerTest, TestDtypesAndDevices);

TYPED_TEST(NormalizedDeconvolutionLayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<Layer<Dtype> > layer(
      new NormalizedDeconvolutionLayer<Dtype>(layer_param));
  ...
  layer.reset(new NormalizedDeconvolutionLayer<Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
...
TYPED_TEST(NormalizedDeconvolutionLayerTest, TestSimpleDeconvolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
...
TYPED_TEST(NormalizedDeconvolutionLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  ...
  NormalizedDeconvolutionLayer<Dtype> layer(layer_param);
