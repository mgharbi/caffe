...
__global__ void XYZ2LABForward(const int n, const Dtype* in, Dtype* out,
    ...
    int height, int width, int chans)
{
    CUDA_KERNEL_LOOP(index, n) {
        ...
        out[src_idx+0*chan_stride] = L;
        out[src_idx+1*chan_stride] = a;
        out[src_idx+2*chan_stride] = b;
    }
}

template <typename Dtype>
__global__ void XYZ2LABBackward(const int n, const Dtype* top_diff, Dtype* bottom_diff, const Dtype* bottom_data,
    double X_ref, double Y_ref, double Z_ref,
    int height, int width, int chans)
{
    CUDA_KERNEL_LOOP(index, n) {
        ...
        Dtype dL = top_diff[src_idx + 0*chan_stride];
        Dtype da = top_diff[src_idx + 1*chan_stride];
        Dtype db = top_diff[src_idx + 2*chan_stride];
...
template <typename Dtype>
void XYZ2LABLayer<Dtype>::Forward_gpu(
...
template <typename Dtype>
void XYZ2LABLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
    ...
    vector<int> shape = bottom[0]->shape();
    int npix = shape[0]*shape[2]*shape[3];
    XYZ2LABBackward<Dtype><<<CAFFE_GET_BLOCKS(npix), CAFFE_CUDA_NUM_THREADS>>>(
...
INSTANTIATE_LAYER_GPU_FUNCS(XYZ2LABLayer);
